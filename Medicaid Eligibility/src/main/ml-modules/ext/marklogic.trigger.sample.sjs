'use strict';
/*
  This trigger is executed for each new document created in the http://example.com/data/household collection.
...
function shapeForTDE(originalInput, corticonOutput) {
    ...
    if (Array.isArray(corticonOutput)) {
        ...
        // fallback: wrap bare household data
...
xdmp.documentInsert(uri, normalizedResult, {
    "collections": "http://example.com/data/household",
    "permissions": [
